  ...
  .action(async (options: UndoOptions) => {
    ...
    try {
      ...
      if (isNaN(commitCount) || commitCount < 1) {
        throw new ValidationError("Number of commits must be a positive integer");
      }

      if (commitCount > 10) {
        throw new ValidationError("Cannot undo more than 10 commits at once for safety");
      }

      logger.info(`🔍 Analyzing last ${pluralize(commitCount, "commit")} to undo...`);
      ...
      if (commits.length < commitCount) {
        logger.warn(`Only ${commits.length} commits available (requested ${commitCount})`);
      }
      ...
      for (let i = 0; i < Math.min(commits.length, commitCount); i++) {
        ...
        logger.info(`  ${i + 1}. ${commit.sha.substring(0, 8)} ${commit.subject}`);
      }
      ...
      if (upstream) {
        ...
        if (!hasUnpushedCommits) {
          ...
          logger.warn("   Undoing pushed commits requires force-push and affects others");
        }
      }
      ...
      if (options.dryRun) {
        ...
        logger.info(`2. Reset HEAD~${commitCount} (keep changes in working directory)`);
      ...
      if (!options.yes) {
        const warningMsg = hasUnpushedCommits 
          ? `Undo last ${pluralize(commitCount, "commit")}? Changes will be preserved in working directory.`
          : `⚠️  SOME COMMITS MAY BE PUSHED! Undo last ${pluralize(commitCount, "commit")}?`;
      ...
      const tagName = `oops/undo-${commitCount}-commits-${timestamp}`;
      
      logger.info("🛡️  Creating safety tag...");
      await git.exec(["tag", "-a", tagName, "-m", `Safety backup before undoing ${commitCount} commits`, "HEAD"]);
      ...
      logger.success(`✅ Successfully undid ${pluralize(commitCount, "commit")}`);
      
      logger.info("\n📝 Summary:");
      logger.info(`  • Undid: ${commitCount} commits`);
      logger.info(`  • Safety tag: ${tagName}`);
      logger.info(`  • Changes preserved in: ${status.staged.length} staged files`);
