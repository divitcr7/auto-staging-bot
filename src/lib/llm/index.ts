import { LLMProvider, ProviderError, Config } from '../../types.js';
...
export class LLMManager {
  ...
  getProvider(providerName?: string): LLMProvider {
    ...
    if (!targetProvider) {
      throw new ProviderError(
        'No AI provider configured. Set OOPS_PROVIDER environment variable or configure in .git-oopsrc'
      );
    }
    ...
    if (!provider.isConfigured()) {
      throw new ProviderError(
        `Provider '${targetProvider}' is not properly configured. ` +
  ...
  private getConfigurationHelp(providerName: string): string {
  ...
  async generateStructuredResponse<T>(
    ...
    try {
      ...
      throw new ProviderError(
        `AI response is not valid JSON: ${response.substring(0, 200)}...`
      );
    }
  }
