...
export class RedactionManager {
    ...
    redactContent(content, options = {}) {
        ...
        for (const secretPattern of this.SECRET_PATTERNS) {
            ...
            if (matches) {
                redactedContent = redactedContent.replace(secretPattern.pattern, secretPattern.replacement);
    ...
    processDiffForAI(diffContent, files, options = {}) {
    ...
    generateRedactionReport(result) {
        const report = [];
