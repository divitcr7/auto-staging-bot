...
import { formatTimestamp, isProtectedBranch } from '../utils.js';
export class SafetyManager {
    git;
    logger;
    constructor(git, logger) {
        this.git = git;
        this.logger = logger;
    }
    // Safety tag creation
    async createSafetyTag(context, target = 'HEAD') {
    ...
    async cleanupOldSafetyTags(daysToKeep = 30) {
    ...
    async checkBranchProtection(branchName, requireConfirmation = true) {
        if (isProtectedBranch(branchName)) {
            const message = `Branch '${branchName}' appears to be protected. This operation could be risky.`;
            this.logger.warn(message);
            if (requireConfirmation) {
                throw new ValidationError(`${message} Use --yes to proceed anyway.`);
            }
        }
    }
    // Push status checks
    async checkPushStatus(branchName) {
    ...
    async warnUnpushedCommits(branchName) {
        const pushStatus = await this.checkPushStatus(branchName);
        if (pushStatus.hasUnpushedCommits) {
            this.logger.warn(`Branch has ${pushStatus.commitCount} unpushed commits. ` +
                `You may need to force-push after this operation.`);
        }
    }
    // Recovery instructions
    generateRecoveryInstructions(safetyTag, operation) {
        return [
    ...
    async validateCleanWorkingDirectory(allowStaged = false) {
        ...
        if (hasUnstaged || hasUntracked) {
            throw new ValidationError('Working directory is not clean. Please commit or stash your changes first.');
        }
        if (hasStaged && !allowStaged) {
            throw new ValidationError('There are staged changes. Please commit or unstage them first.');
        }
    }
    ...
    // Upstream validation
    async validateUpstream(branchName) {
        const currentBranch = branchName || await this.git.getCurrentBranch();
        const upstream = await this.git.getUpstream(currentBranch);
        if (!upstream) {
        ...
        try {
            ...
            throw new ValidationError(`Upstream '${upstream}' does not exist. Please fetch or set a valid upstream.`);
        }
        return upstream;
    }
    // Validate commit exists and is accessible
    async validateCommit(sha, context) {
    ...
    async checkForWorkLoss(targetSha, currentBranch) {
    ...
    async performPreOperationChecks(options = {}) {
        const { requireCleanWorkingDir = true, allowStaged = false, checkBranchProtection = true, checkUpstream = false, branchName, operation = 'operation', } = options;
