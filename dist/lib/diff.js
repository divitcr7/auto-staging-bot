import { getTopLevelDirectory, formatBytes, bytesToKb } from '../utils.js';
export class DiffManager {
    ...
    async getStagedDiffContent(options = {}) {
        ...
        if (sizeKb > maxKb) {
            ...
            content += '\n\n[... content truncated ...]';
            truncated = true;
            this.logger.warn(`Diff content truncated from ${formatBytes(size)} to ${formatBytes(maxBytes)}`);
        }
        return {
            content,
            size,
    ...
    async getDiffForFiles(files, options = {}) {
    ...
    async applyPatch(patchContent, options = {}) {
        ...
        try {
            ...
            await import('fs/promises').then(fs => fs.writeFile(tempFile, patchContent, 'utf8'));
            try {
    ...
    async getChangeContext(file, lineNumber, contextLines = 3) {
        try {
            ...
            const before = lines.slice(Math.max(0, targetIndex - contextLines), targetIndex);
            const change = lines[targetIndex] || '';
            const after = lines.slice(targetIndex + 1, Math.min(lines.length, targetIndex + 1 + contextLines));
    ...
    validateDiffForSafety(diffContent) {
        const warnings = [];
