...
export const aiCommitCommand = new Command('commit')
    ...
    try {
        ...
        if (!llm.isAnyProviderConfigured()) {
            throw new ValidationError('No AI provider configured. Please set up OpenAI, Anthropic, Azure OpenAI, or Ollama.\n' +
        ...
        const redactionResult = redact.processDiffForAI(diffResult.content, stagedFiles, {
            maxKb,
        ...
        const response = await llm.generateWithSystemPrompt(systemPrompt, contextInfo, {
        ...
        if (options.apply) {
            ...
            const shouldCommit = await confirm('Commit with this message?', false, options);
            if (shouldCommit) {
                const fullMessage = suggestion.body
                    ? `${suggestion.title}\n\n${suggestion.body}`
                    : suggestion.title;
