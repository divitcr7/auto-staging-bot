...
export const revertMergeCommand = new Command('revert-merge')
    ...
    .action(async (mergeSha, options, command) => {
    ...
    try {
        ...
        if (isNaN(mainline) || mainline < 1 || mainline > parents.length) {
            throw new ValidationError(`Invalid mainline ${options.mainline}. Must be between 1 and ${parents.length}`);
        }
        ...
        if (!options.yes) {
            ...
            const confirmed = await confirm(`Revert merge ${mergeSha.substring(0, 8)} with mainline ${mainline}?`, false, options);
            if (!confirmed) {
...
//# sourceMappingURL=revertMerge.js.map